...
//Stack pre:  bytes32 orderID, bytes32 buyerAddress, bytes32 sellerAddress, uint256 ipAddress
func opOrder(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {

	orderID, buyerAddress, sellerAddress, ipAddress := stack.pop(), stack.pop(), stack.pop(), stack.pop()
	if interpreter.evm.Order != nil {
		interpreter.evm.Order(orderID, buyerAddress, sellerAddress, ipAddress)
	}
